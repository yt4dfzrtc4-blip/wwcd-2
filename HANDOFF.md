...
Dernier déploiement prod OK (23/08/2026), incluant perf par compte + fiscalité + Monte Carlo + PWA (logo réel, icônes affinées) + rebrand (bleu `#004AAD` + fond crème assoupli `#F7F4EE` + logo dans la topbar). Migration 005 exécutée côté Supabase, confirmée par l'utilisateur. Reste à confirmer : validation utilisateur du fond assoupli + logo topbar, rendu de l'icône PWA après réinstallation sur l'écran d'accueil, et les 4 tickers en échec de récupération de cours (APO, BSDE.DE, ALGBE.PA, ASML.DE — probablement mal orthographiés sauf APO, à vérifier dans `/assets`).
